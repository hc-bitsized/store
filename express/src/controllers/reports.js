...
function _getRequestInfo(req) {
    ...
    if (req.query.period) {
        request.filters.startDate = moment(req.query.period).toJSON()
        request.filters.endDate = moment(req.query.period).endOf('month').subtract(1, 'day').subtract(3, 'hour')
    } else {
        if (req.query.startDate) request.filters.startDate = moment(req.query.startDate).toJSON()
        if (req.query.endDate) request.filters.endDate = moment(req.query.endDate).toJSON()
    }
